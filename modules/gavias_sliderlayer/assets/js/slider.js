...
! function(a) {
    ...
    function f() {
        a("#gavias_slider_single").find("div").remove(), a("#gavias_list_layers").find("li").remove(), "undefined" == typeof $layers && ($layers = new Array), a($layers).each(function(a) {
            $layers[a] && 1 != $layers[a].removed && h(a)
        }), a(".layer-option").val(""), "undefined" != typeof $layers[0] && k(0)
    }
    ...
    function h(b) {
        ...
        h.addClass("caption"), "undefined" == typeof $layers[b].text_style && ($layers[b].text_style = "text"), "text" == $layers[b].type && h.addClass($layers[b].text_style);
        var i = "";
        switch ($layers[b].type) {
            case "image":
                var j = drupalSettings.gavias_sliderlayer.base_url + $layers[b].image_uri;
                i = '<img src="' + j + '" />';
                var n = new Image;
                n.onload = function() {
        ...
        $layers[b].custom_css && o.attr("style", $layers[b].custom_css), $layers[b].custom_class && o.addClass($layers[b].custom_class), o.html(i), h.append(o);
        ...
        h.mousedown(function() {
            ...
            containment: "parent",
            drag: function(c, d) {
                a("input[name=left]").val(d.position.left), a("input[name=top]").val(d.position.top), l(b, d.position.top, d.position.left)
            },
            grid: [5, 5]
        }).resizable({
    ...
    function k(b) {
        $cxt = b, a(".layer").removeClass("selected"), a("#layer-" + b).addClass("selected"), a("ul#gavias_list_layers").find("li").removeClass("active"), a("ul#gavias_list_layers").find("li[index=" + b + "]").addClass("active"), a(".layer-option").each(function(c) {
            "undefined" != typeof $layers[b][a(this).attr("name")] ? "data_time_end" == a(this).attr("name") || "data_time_start" == a(this).attr("name") ? a(this).val($layers[b][a(this).attr("name")]).trigger("change") : a(this).val($layers[b][a(this).attr("name")]) : a(this).val("")
        });
        var c = $layers[b].select_content_type;
        j(c), a(".select-content-type").change(function() {
            var b = a(this).val();
            j(b)
        })
    }
    ...
    function n(b) {
        if (a("#layer-" + b).remove(), $layers[b].removed = 1, a("ul#gavias_list_layers").find("li[index=" + b + "]").remove(), b == $cxt && a("ul#gavias_list_layers li").length > 0) {
            var c = parseInt(a("ul#gavias_list_layers").find("li:first").attr("index"));
            k(c)
        }
    }
    ...
    a(window).load(function() {
        ...
        d.noUiSlider.on("update", function(a, b) {
            ...
            a(".layer[id=layer-" + $cxt + "] .inner").attr("class", "inner " + $layers[$cxt].custom_class), a(".layer[id=layer-" + $cxt + "] .inner").addClass(a(this).val())
            ...
            $layers[$cxt].custom_class = a(this).val(), a("#layer-" + $cxt).find(".inner").attr("class", "inner"), a("#layer-" + $cxt).find(".inner").addClass(a(this).val())
